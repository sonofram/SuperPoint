...
class SuperpointPmMatchDataset(BaseDataset):
    default_config = {
        'dataset': 'pmpatches',  # or 'coco'
    ...
    def _init_dataset(self, **config):
        ...
        for path in folder_paths:
            ...
            if file_name+'.jpg' not in filter_list_names:
        ...
        if config['truncate']:
            ...
            file_name = file_name[:config['truncate']]
    ...
    def _get_data(self, files, split_name, **config):
        ...
        warped_images = warped_images.map(lambda path: tf.py_func(_read_image,
                                                                  [path],
                                                                  tf.uint8))      
        names = tf.data.Dataset.from_tensor_slices(files['name'])
