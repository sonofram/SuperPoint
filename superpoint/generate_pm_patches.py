...
if __name__ == '__main__':
    ...
    # Take sample images
    init_image_paths = Path(DATA_PATH, 'PM/train_images/')
    sample_paths = list(init_image_paths.iterdir())
    base_path = Path(DATA_PATH, 'PM/test_images/')
    
    files = [f for f in base_path]
        
    random_files = np.random.choice(files, int(len(files)*.1))
    
    for f in random_files:
        shutil.copy(f, base_path) 
    
    
    image_paths = list(base_path.iterdir())
    output_dir = Path(DATA_PATH, 'PM/patches/')
    ...
    print("Generating patches of Pm test...")
    sess = tf.InteractiveSession()
    for path in image_paths:
        new_path = Path(output_dir, path.stem)
